...
<script type="text/javascript">
    ...
    function focusIt()
    {
      document.ingresa_pac.doc_paciente.focus();
    }
</script>
...
if ($doc_paciente <> "")
{
	$sql = "SELECT * FROM paciente where rut = '".$doc_paciente."'";
	if ($tipodocumento == 3){
                                        ...
                                        if ($rut_paciente <> 0 or $cta_cte <> 0)
                                        {
                                            ...
                                            while($list_paciente = mysql_fetch_array($query2)){
												...
												switch($list_paciente['ATEprocede']){
													...
													case 3: $procedencia="Hospitalizaci�n"; break;
													case 4: $procedencia="A.Primaria"; break;
													case 5: $procedencia="Cl�nica"; break;
                                ...
                                <div style="width:780px;height:285px;overflow:auto;">
                                    ...
                                    echo "<table id='anatomia' border='1' cellpadding='3px' cellspacing='0px'>";
                                        if ($id_paciente <> 0 or $cta_cte <> 0)
                                        {
										    if($opcion_1 == 'on')
											{
                                            	$sql = "SELECT DISTINCT ana.regAId, ana.regAtipoExamen, ana.regAFechaSolicitud, ana.regAFechaTomaPap, ana.regAEstadoDiagnostico,
														...
														WHERE pacId = '".$id_paciente."'
														AND regATipoExamen = 'B' ORDER BY regAId DESC";
											}
											else
											{
                                            	$sql = "SELECT DISTINCT ana.regAId, ana.regAtipoExamen, ana.regAFechaSolicitud, ana.regAFechaTomaPap, ana.regAEstadoDiagnostico,
                                                ...
                                                <tr align="left" height="25px" style="font-size:10px; font-family:Verdana, Arial, Helvetica, sans-serif; cursor:pointer;"
													<? $regAId = $list_paciente['regAId'];
														if($list_paciente['regAtipoExamen'] == 'P') 
                                                            echo " onClick='javascript: window.open(\"http://".$_SERVER['SERVER_NAME']."/anatomiaPatologica/servlet/ainformepap?$regAId\",\"\",\"titlebars=0,toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,height=800,width=1000\")';";
														else if($list_paciente['regAtipoExamen'] == 'B' and $list_paciente['regAEstadoDiagnostico'] == 'C')
															echo " onClick='javascript: window.open(\"http://".$_SERVER['SERVER_NAME']."/anatomiaPatologica/servlet/adiagnostico2?$regAId\",\"\",\"titlebars=0,toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,height=800,width=1000\")';";
														else if($list_paciente['regAtipoExamen'] == 'M')
															echo " onClick='javascript: window.open(\"http://".$_SERVER['SERVER_NAME']."/anatomiaPatologica/servlet/adiagnosticosmiscelaneos?$regAId\",\"\",\"titlebars=0,toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,height=800,width=1000\")';";
                                                    ?>>
                                    ...
                                    echo "<table id='epicrisis' border='1' cellpadding='3px' cellspacing='0px'>";
                                        if ($id_paciente <> 0 or $cta_cte <> 0)
                                        {
                                            ...
                                            while($list_paciente = mysql_fetch_array($query2)){?>
                                                ...
                                                <td width="150"><? if($list_paciente['epiTipoId'] == 'P') echo "Pedi�trica"; else if($list_paciente['epiTipoId'] == 'M') echo "M�dica"; ?></td>
...
<form name="ingresa_pac" method="get" action="hist_clinico.php">
    ...
    <table width="780px" border="0" cellspacing="1" cellpadding="1">
            <tr>
                ...
                <td>
                    ...
                    <option value=1 <? if ($tipodocumento == 1) { echo "selected"; } ?> />Cta-Cte
			        <option value=2 <? if ($tipodocumento == 2) { echo "selected"; } ?> />Rut
                    <option value=3 <? if ($tipodocumento == 3) { echo "selected"; } ?> />Ficha
                    </select>		                
                </td>
                <td><span id="spry_doc_paciente">
                <input size="9" type="text" name="doc_paciente" value="<? echo $doc_paciente ?>" />
                <span class="textfieldRequiredMsg">???</span><span class="textfieldMinValueMsg">???</span></span>
                  <input size="1" type="text" name="pdv"  value="<?php echo ValidaDVRut($rut_paciente); ?>" disabled="disabled" >
					<input type="submit" value="Aceptar" >
				  <input type="Button" value="Buscar" onclick="window.location.href='<? echo"busquedapacientes.php"; ?>'; parent.GB_hide(); " >
					Prevision <input size="20" type="text" name="prevision" value="<? echo $prevision; ?>" disabled="disabled" >
              </td>
	            <td width="45px">Fono1</td>
                <td width="100px">
                    <input size="12" type="text" name="fono1_paciente" value="<? echo $fono1_paciente; ?>" disabled="disabled" >
                </td>
                <td width="10px">&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>Nombre</td>
                <td>
                    <input size="79" type="text" name="nom_paciente" value="<? echo $nom_paciente; ?>" disabled="disabled" >
                </td>
	            <td width="45px">Fono2</td>
                <td>
                	<input size="12" type="text" name="fono2_paciente" value="<? echo $fono2_paciente; ?>" disabled="disabled" >
                </td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>Direcci�n</td>
                <td>
                <input size="79" type="text" name="direc_paciente" value="<? echo $direc_paciente; ?>" disabled="disabled" >
                </td>
	            <td width="45px">Fono3</td>
                <td>
                	<input size="12" type="text" name="fono3_paciente" value="<? echo $fono3_paciente; ?>" disabled="disabled" >
                </td>
                <td>&nbsp;</td>
            </tr>
    </table>
...
<script type="text/javascript">
<!--
var spry_doc_paciente = new Spry.Widget.ValidationTextField("spry_doc_paciente", "integer", {validateOn:["change"], useCharacterMasking:true, minValue:1});
//-->
</script>
