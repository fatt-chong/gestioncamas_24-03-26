...
$fecha_alta = cambiarFormatoFecha2($fecha_egreso);
...
if ($cod_destino == 0){
	...
	$l_servicios = mysql_fetch_array($query);
	$destino = $l_servicios['servicio'];

}


if ($cod_procedencia == 0 or $cod_procedencia == 50){ $tipo_traslado = 1; }
else { 	$tipo_traslado = 2; }



if ($cod_destino > 100){ $tipo_traslado = $cod_destino; }
	...
	$resultado = mysql_query( "UPDATE hospitalizaciones SET
	...
	if ($resultado)
		...
		{
		echo "El Cambio de Datos Falló, Intentelo Nuevamente";
		?>
...
else
{
	...
	$resultado = mysql_query( "DELETE FROM hospitalizaciones WHERE id = $id_hosp "  );
	...
	{
		echo "La Eliminacion de Registro Falló, Intentelo Nuevamente";
		?>
        <input type="Button" value=    "       Volver       " onclick="window.location.href='mantencion_censo.php<? echo $pag_ant ?>'; parent.GB_hide(); " />
		<?
	}
